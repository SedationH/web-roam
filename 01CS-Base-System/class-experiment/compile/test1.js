...
const wipeSpace = new Transform({
  transform: (chunk, encoding, cb) => {
    cb(null, chunk.toString().replace(/\s+/g, " "))
  },
})
    ...
    for (let i = 0; i < source.length; ) {
      ...
      if (/[a-zA-Z]/.test(char)) {
        ...
        const criticalPosition =
          opsitePosition === -1 ? source.length : i + opsitePosition
        ...
        // 处理开头是数字的情况
        const opsitePosition = source.slice(i).search(/[^0-9\.]/)
        const criticalPosition =
          opsitePosition === -1 ? source.length : i + opsitePosition
        const word = source.slice(i, criticalPosition)
...
readStream.pipe(wipeSpace).pipe(lexical).pipe(writeStream)
